...
void Init() {
	SetConsoleTitleA("小传说 1.07");
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0f);
	system("cls");
	srand(time(0));
	g_nGameStatus = 0;
	printf("小传说(Hero Tales) v1.07\n");
	printf("Developed by Peter Winchester, 2025.\n\n");
	...
	for (int i = 1; i <= 1; i++) {
		...
		card[iCurrentCard].nNumber        = iCurrentCard;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Grail;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = ROYAL;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Minister;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = ROYAL;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Princess;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = ROYAL;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Warrior_Royal;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = ROYAL;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_King;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = ROYAL;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_SaintSword;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_BlackSmith;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Fighter;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Monk;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Nun;
	}
	printf(".");
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Businessman;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_BabyCrane;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Dancer;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Master;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Guard;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_HolyKnight;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Berserker;
	}
	printf(".");
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Warrior_Lv4;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Warrior_Lv5;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_BlackKnight;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Demon;
	}
	printf(".");
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_FireDragon;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = DAEMON;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Tyrant;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = DAEMON;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Mimic;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = DAEMON;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Blade;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nFire          = FIRE;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_DragonCub;
	}
	printf(".");
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_DragonFlame;
	}
	for (int i = 1; i <= 1; i++) {
		iCurrentCard++;
		card[iCurrentCard].nFire          = FIRE;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_DragonEgg;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Slime;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Bomb;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Bat;
	}
	printf(".");
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Bone;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Wolfman;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_HarleyQuinn;
	}
	for (int i = 1; i <= 2; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Witch;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Goblin;
	}
	for (int i = 1; i <= 4; i++) {
		iCurrentCard++;
		card[iCurrentCard].nArmory        = NO_ARMORY;
		...
		card[iCurrentCard].pfnCalcPref    = CalcPref_Tauren;
	}
	printf(".Done!\n");
}
...
void Help() {
	while (1) {
		...
		char ch = _getch();
		if (ch == '0') break;
		else if (ch == '1') DescribeCards();
		else if (ch == '2') DisplayRules();
		else if (ch == '3') {
			printf("小传说(Hero Tales) v1.07\n");
			printf("Developed by Peter Winchester, 2025.\n\n");
			system("pause");
